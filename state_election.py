...
with cols_a[1]:
    lost_faith = st.number_input('Lost of Faith in BN/PH %', min_value=0, max_value=100, value=5, step = 1)
    lost_faith /= 100
...
with cols_a[3]:
    ph_to_bn = st.number_input('Transferability PH to BN %', min_value=0, max_value=100, value=24, step = 1)
    ph_to_bn /= 100
...
df, scenario_df = run_election_model(state, lost_faith, bn_to_ph, ph_to_bn)
